#!/usr/bin/env python3
"""Download new novels - fixed URL encoding."""
import urllib.request, urllib.parse, os, threading, re
...
def encode_url(url):
    """Percent-encode Chinese characters in URL path only, not query params."""
    parts = url.split('?', 1)
    path = parts[0]
    query = '?' + parts[1] if len(parts) > 1 else ''
    # Encode non-ASCII chars in path
    encoded_path = urllib.parse.quote(path, safe='/:@!$&\'()*+,;=-._~%')
    return encoded_path + query
...
def dl(name, url):
    ...
    try:
        ...
        urllib.request.urlretrieve(encoded, out)
